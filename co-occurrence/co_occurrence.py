...
def distinct_words(corpus):
    ...
    allWords =  [word for myList in corpus for word in myList]
    corpus_words = sorted(list(set(allWords)))
...
def compute_co_occurrence_matrix(corpus, window_size=4):
    ...
    #Initialize the word to index dict
    word2Ind = {newword:nindex for nindex, newword in enumerate(words)}
    #Initialize the matrix with zeros
    M = np.zeros(shape=(num_words, num_words), dtype=np.float64)
    for myList in corpus:
        for mindex, mword in enumerate(myList):
            new_winIndex = 0
            winIndex = 0
            if window_size !=0:
                for winIndex in range(-window_size, window_size + 1):
                #Check we are not at the center word
                    if winIndex !=0:
                        new_winIndex = mindex + winIndex
                        # Check we are within the myList
                        if new_winIndex >= 0 and new_winIndex < len(myList):
                            M[word2Ind[mword],word2Ind[myList[new_winIndex]]] += 1
