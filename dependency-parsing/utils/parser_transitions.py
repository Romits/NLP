class PartialParse(object):
    def __init__(self, sentence):
        ...
        ###
        self.stack = ["ROOT"]
        self.buffer = self.sentence.copy() # avoid the original sentence from getting modified
        self.dependencies = []
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for parse, transition in zip(minibatch, pred_transitions):
            parse.parse_step(transition)
            if (not parse.buffer and len(parse.stack) == 1):
                unfinished_parses.remove(parse)
    dependencies = [myparse.dependencies for myparse in partial_parses]
    return dependencies
