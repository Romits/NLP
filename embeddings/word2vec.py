...
def naive_softmax_loss_and_gradient(
    ...
    y_hat_all = softmax(np.dot(center_word_vec, outside_vectors.T))
    loss = -np.log(y_hat_all[outside_word_idx])
    delta = y_hat_all.copy()
    delta[outside_word_idx] -= 1.0
    grad_center_vec = np.dot(delta, outside_vectors)
    grad_outside_vecs = np.dot(delta[np.newaxis,:].T, center_word_vec[np.newaxis, :])
...
def skipgram(current_center_word, window_size, outside_words, word2ind,
    ...
    curr_cw_index = word2ind[current_center_word]
    curr_cw_vec = center_word_vectors[curr_cw_index]
    for outside_word in outside_words:
        outside_word_index = word2ind[outside_word]
        deltaloss, grad_center_vec, grad_outside_vec = word2vec_loss_and_gradient(curr_cw_vec, outside_word_index, outside_vectors, dataset)
        loss +=deltaloss
        grad_center_vecs[curr_cw_index] +=grad_center_vec
        grad_outside_vectors += grad_outside_vec
