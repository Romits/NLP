...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(input_size=hidden_size+embed_size, hidden_size=self.hidden_size, bias=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        enc_hidden_projs = self.att_projection(enc_hiddens)
        Y = self.model_embeddings.target(target_padded)
        for Y_t in torch.split(Y,split_size_or_sections=1, dim=0):
            Y_t = torch.squeeze(Y_t, dim=0)
            Ybar_t = torch.cat((Y_t, o_prev), dim=1)
            dec_state, o_t,_ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hidden_projs, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # squeeze to change (b,1,2h) to (b,2h)
