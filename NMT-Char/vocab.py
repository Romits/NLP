...
class VocabEntry(object):
    ...
    def words2charindices(self, sents):
        ...
        return [[[self.char2id["{"]] + [self.char2id[mychar] for mychar in myword] + [self.char2id["}"]] for myword in mysentence] for mysentence in sents]
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        ###     previous parts
        sents_encoded = self.words2charindices(sents)
        sents_padded = pad_sents_char(sents_encoded, self.char2id['<pad>']) # (batch_size, max_sentence_len, max_word_len)
        return torch.tensor(sents_padded).permute(1,0,2).to(device) # (max_sent_length, batch_size, max_word_len)
